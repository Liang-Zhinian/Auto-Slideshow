...
using AutoSlide.Models.Gfycat;
using AutoSlide.Authentication;
using System.Net.Http.Headers;
using AutoSlide.Models.Imgur;
...
namespace AutoSlide.Views
{
    ...
    public sealed partial class Slideshow : Page
    {
        ...
        private Task<List<MediaUrl>> GenerateImages(object[] param, CancellationToken token) {
            string type = param[0] as string;
            List<string> urls = param[1] as List<string>;
            if (type == "Local") return GenerateLocalImages(urls, token);
            else if (type == "Reddit") return GenerateRedditImages(urls, token);
        ...
        private void enableRandom(object sender, RoutedEventArgs e)
        {
            CheckBox checkbox = sender as CheckBox;
            medialist.Shuffle();
        }
